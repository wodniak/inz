...
Maze::Maze(vector<Graph_Node*> & all_nodes) 
{
	...
	//first encountered node in last line is start node 
		...
		{
			if (g -> operator[](i)->getCenter()->y == 15 && end_node == nullptr && g -> operator[](i)->getCenter()-> x > 100)	//HARDCODED
			{
				end_node = g -> operator[](i);
				curr_node = g -> operator[](i);
			}
			else if (g -> operator[](i)->getCenter()->y <= 420 && g -> operator[](i)->getCenter()-> x > 100) //HARDCODED last sliding window line!!
			{
...
tuple<double, double, bool, int> Maze::calc_dist_to_line(cv::Point2i & tank_pos,int & angle)
{
	//get next node 
	...
	//cv::Mat dst();
	// draw the box
	//cv::rectangle(dst, tank_rect, cv::Scalar(255, 0, 0), 1, 1, 0);
	//imshow("dst", dst);
	cout << "tank_rect : " << tank_rect.x << " " << tank_rect.y << endl;
	cout << "Arrived to : " << node_rect.x << " " << node_rect.y << endl;
	...
	if((tank_rect & node_rect).area() > 0)
	{
		...
		/*
		if (is_corner_left(curr_node)) {
			corner = 1;
		}
		else if (is_corner_right(curr_node)) {
			corner = 2;
		}
		*/
		//get next node 
	...
	return make_tuple(dist_to_node,dist_perpendicular, tank_turn_right, corner);
}
...
void Maze::draw_solution(cv::Mat& img)
{
	...
	while (true)
	{
		...
		else
		{
			...
			//cv::line(img, *(node->getCenter()), *(next->getCenter()), cv::Scalar(255,0,0), 4, 6, 0);
